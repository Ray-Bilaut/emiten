...
class Infographic extends APP_Webtools {

	public function __construct()
	{
        parent::__construct();
        $this->_template_master_data['page_title'] = 'Infographic';
    ...
    public function index($error='')
    {
        ...
        $this->_data['list'] = $this->db->get_where('infographic', array('is_delete'=>0))->result();
    ...
    public function addprocess()
    {
        ...
        $img = $this->mfupload->image('image','./uploads/infographic/',true,'image_');
        if( $img->status==400 ){
        ...
        if ($isThumbSame) {
            ...
            $img = $this->mfupload->image('thumb','./uploads/infographic/',true,'thumb_');
            if( $img->status==400 ){
                $this->session->set_flashdata('error', $img->message);
                redirect(ADMIN_URL."/{$this->router->class}/add/error");
            }else{
        ...
        if ($isImageMobileSame) {
            ...
            $img = $this->mfupload->image('image_mobile','./uploads/infographic/',true,'mobile_');
            if( $img->status==400 ){
                $this->session->set_flashdata('error', $img->message);
                redirect(ADMIN_URL."/{$this->router->class}/add/error");
            }else{
        ...
        if ($this->db->insert('infographic', $data)) {
            $new_id = $this->db->insert_id();

            $this->notif_model->set_push_queue([
                'user_id' => 0,
                'referal_type' => 'infographic',
                'referal_id' => $new_id,
                'title' => $title,
                'url' => 'infographic/'.$data['slug_url'],
    ...
    protected function slug_url($word,$edit=false,$edit_id=0)
    {
        ...
        while(!$get){
            ...
            if($edit){
                $res = $this->db->get_where('infographic',['slug_url' => $url.$subfix, 'id <>' => $edit_id])->num_rows();
            }else{
                $res = $this->db->get_where('infographic',['slug_url' => $url.$subfix])->num_rows();
            }
    ...
    public function edit($idx,$error='')
    {
        ...
        $data = $this->db->get_where('infographic',['id'=>$idx])->row();
    ...
    public function editprocess($idx)
    {
        ...
        $img = $this->mfupload->image('image', './uploads/infographic/', false, 'image_');
        if ($img->status==400) {
            $this->session->set_flashdata('error', $img->message);
            redirect(ADMIN_URL."/{$this->router->class}/edit/{$idx}/error");
        ...
        if ($isThumbSame) {
            ...
            $img = $this->mfupload->image('thumb', './uploads/infographic/', false, 'thumb_');
            if ($img->status==400) {
                $this->session->set_flashdata('error', $img->message);
                redirect(ADMIN_URL."/{$this->router->class}/edit/{$idx}/error");
            } elseif ($img->status==200) {
        ...
        if ($isImageMobileSame) {
            ...
            $img = $this->mfupload->image('image_mobile', './uploads/infographic/', false, 'mobile_');
            if ($img->status==400) {
                $this->session->set_flashdata('error', $img->message);
                redirect(ADMIN_URL."/{$this->router->class}/edit/{$idx}/error");
            } elseif ($img->status==200) {
        ...
        if ($this->db->update('infographic', $data_new, array('id'=>$idx))) {
            $this->db->delete('tag_relation', ['infographic_id' => $idx]);
    ...
    public function delete($id)
    {
        ...
        $res = $this->db->update('infographic', ['is_delete' => 1], ['id' => $id]);

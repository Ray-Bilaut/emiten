...
class News extends APP_Webtools {

	public function __construct()
	{
        parent::__construct();
        $this->_template_master_data['page_title'] = 'News';
        $this->load->model("notif_model");
	}

    public function index($error='', $year='2021')
    {
        ...
        $this->_data['years'] = $this->getyears();
        $this->_data['list'] = $this->db->get_where('news', array('is_delete'=>0, 'publish_date >='=> $year.'-01-01 00:00:00', 'publish_date <='=> $yearint.'-01-01 00:00:00'))->result();
    ...
    private function getyears() {
        ...
        $now = (int)$now;
        $years = [];
        for ($i=0; $i <= ($now-$minYear); $i++) {
            array_push($years, $minYear+$i);
        }
    ...
    public function add($error='')
    {
        ...
        //             child.*
        //         FROM
        //         {$this->db->dbprefix('category')} child
        //         WHERE 
        //             child.is_delete=0
        ...
        //         JOIN
        //             {$this->db->dbprefix('category')} cat
        //         ON
        //             c.id=cat.parent_id
        //         WHERE 
        //             c.is_delete=0
		// 		";
        // $this->_data['categories'] = $this->db->query($categoryQry)->result();
        $this->_data['categories'] = $this->db->get_where('category', array('is_delete'=>0, 'parent_id'=>0))->result();
        $this->_data['authories'] = $this->db->get_where('author')->result();
    ...
    public function addprocess()
    {
        ...
        $data = [
            ...
            'is_highlight' => intval($this->input->post('is_highlight')),
            'is_recommendation' => intval($this->input->post('is_recommendation')),
            'is_trending' => intval($this->input->post('is_trending')),
        ...
        $file = $this->mfupload->pdf('file_attachment','./uploads/file_news/',false,'file_');
        if( $file->status==400 ){
            $this->session->set_flashdata('error', $file->message);
            redirect(ADMIN_URL."/{$this->router->class}/add/error");
        }else{
            $data['file_attachment'] = empty($file->pdf) ? '' : $file->pdf;
        }

        $img = $this->mfupload->image('image','./uploads/news/',true,'image_');
        if( $img->status==400 ){
        ...
        if ($isThumbSame) {
            ...
            $img = $this->mfupload->image('thumb','./uploads/news/',true,'thumb_');
            if( $img->status==400 ){
                $this->session->set_flashdata('error', $img->message);
                redirect(ADMIN_URL."/{$this->router->class}/add/error");
            }else{
        ...
        if ($isImageMobileSame) {
            ...
            $img = $this->mfupload->image('image_mobile','./uploads/news/',true,'mobile_');
            if( $img->status==400 ){
                $this->session->set_flashdata('error', $img->message);
                redirect(ADMIN_URL."/{$this->router->class}/add/error");
            }else{
    ...
    protected function slug_url($word,$edit=false,$edit_id=0)
    {
        ...
        while(!$get){
            ...
            if($edit){
                $res = $this->db->get_where('news',['slug_url' => $url.$subfix, 'id <>' => $edit_id])->num_rows();
            }else{
                $res = $this->db->get_where('news',['slug_url' => $url.$subfix])->num_rows();
            }
    ...
    public function edit($idx,$error='')
    {
        ...
        $data = $this->db->get_where('news',['id'=>$idx])->row();

        if( empty($data->id)){
            redirect(ADMIN_URL."/{$this->router->class}");
        }

        $this->_data['data'] = $this->prepare($data);
        $this->_data['id'] = $idx;
        $this->_data['error'] = $error;
        $this->_data['error_msg'] = $this->session->flashdata('error');
        $this->_data['success_msg'] = $this->session->flashdata('success');
    ...
    public function editprocess($idx)
    {
        ...
        // $data_new['author_id'] = $this->_template_master_data['user']->id;
        ...
        if ($file->status==400) {
            $this->session->set_flashdata('error', $file->message);
            redirect(ADMIN_URL."/{$this->router->class}/edit/{$idx}/error");
        } elseif ($file->status==200) {
            $data_new['file_attachment'] = empty($file->pdf) ? '' : $file->pdf;
        }

        $img = $this->mfupload->image('image', './uploads/news/', false, 'image_');
        if ($img->status==400) {
            $this->session->set_flashdata('error', $img->message);
            redirect(ADMIN_URL."/{$this->router->class}/edit/{$idx}/error");
        ...
        if ($isThumbSame) {
            ...
            $img = $this->mfupload->image('thumb', './uploads/news/', false, 'thumb_');
            if ($img->status==400) {
                $this->session->set_flashdata('error', $img->message);
                redirect(ADMIN_URL."/{$this->router->class}/edit/{$idx}/error");
            } elseif ($img->status==200) {
        ...
        if ($isImageMobileSame) {
            ...
            $img = $this->mfupload->image('image_mobile', './uploads/news/', false, 'mobile_');
            if ($img->status==400) {
                $this->session->set_flashdata('error', $img->message);
                redirect(ADMIN_URL."/{$this->router->class}/edit/{$idx}/error");
            } elseif ($img->status==200) {
        ...
        if ($this->db->update('news', $data_new, array('id'=>$idx))) {
            $this->db->delete('tag_relation', ['news_id' => $idx]);

            if ($this->notif_model->get_by_referal('news', $idx) == FALSE) {
                $this->notif_model->set_push_queue([
                    'user_id' => 0,
                    'referal_type' => 'news',
                    'referal_id' => $idx,
                    'title' => $title,
                    'url' => 'news/'.$data_new['slug_url'],
                    'message' => substr(strip_tags($description), 0, 50).'...'
                ], $publishDateTime);
            }
    ...
    public function delete($id)
    {
        ...
        $res = $this->db->update('news', ['is_delete' => 1], ['id' => $id]);
    ...
    private function prepare($row) {
        $dir = "news/";
        $strDir = explode("/", $row->image);
        if (count($strDir) > 2) {
            $dir = "";
        }
        $row->image_url = base_url('uploads/'.$dir.$row->image);
        $row->image_mobile_url = base_url('uploads/'.$dir.$row->image_mobile);
        $row->thumb_url = base_url('uploads/'.$dir.$row->thumb);
